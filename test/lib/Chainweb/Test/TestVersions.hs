...
testVersionTemplate v = v
    ...
    & versionMaxBlockGasLimit .~ Bottom (Just 2_000_000)
    & versionBootstraps .~ [testBootstrapPeerInfos]
    & versionVerifierPluginNames .~ AllChains (Bottom mempty)
...
timedConsensusVersion g1 g2 = buildTestVersion $ \v -> v
    ...
    & versionUpgrades .~ AllChains HM.empty
    & versionGraphs .~ (BlockHeight 8, g2) `Above` (Bottom g1)
    & versionCheats .~ VersionCheats
        { _disablePow = True
        , _fakeFirstEpochStart = True
        , _disablePact = True
...
slowForkingCpmTestVersion g = buildTestVersion $ \v -> v
    ...
    & versionVerifierPluginNames .~ AllChains
        (Bottom $ Set.fromList $ map VerifierName ["allow", "hyperlane_v3_announcement", "hyperlane_v3_message"])
